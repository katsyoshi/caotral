...
class LinkerNeededTest < Test::Unit::TestCase
  ...
  def test_needed_in_shared_object
    @output = "libneeded.so"
    compile_path = Pathname.new("sample/C/needed.c").to_s
    IO.popen(["gcc", "-fPIC", "-c", "-o", @input, "%s" % compile_path]).close

    Caotral::Linker.link!(inputs: [@input], output: @output, linker: "self", needed: ["libc.so.6"], executable: false, shared: true)
  ...
  def test_start_in_shared_object
    @input = "start.o"
    @output = "libstart.so"
